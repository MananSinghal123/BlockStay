...
// export const getAptogotchi = async (aptogotchiObjectAddr: string): Promise<[string, AptogotchiTraits]> => {
...
export const listAptogotchi = async (sender: Account, aptogotchiObjectAddr: string) => {
  const rawTxn = await aptos.transaction.build.simple({
    sender: sender.accountAddress,
    data: {
      function: `${import.meta.env.VITE_MODULE_ADDRESS}::marketplace::list_with_fixed_price`,
      typeArguments: [APT],
...
export const buyAptogotchi = async (sender: Account, listingObjectAddr: string) => {
  const rawTxn = await aptos.transaction.build.simple({
    sender: sender.accountAddress,
    data: {
