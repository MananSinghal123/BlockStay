...
//     }
    ...
    fun test_happy_path(
        ...
        assert!(seller_addr2 == seller_addr, 0); // The seller is correct
        
        launchpad::purchase<aptos_coin::AptosCoin>(purchaser, object::convert(listing));

        assert!(object::owner(token) == purchaser_addr, 0); // Token transferred to purchaser
        assert!(coin::balance<aptos_coin::AptosCoin>(seller_addr) == 10500, 0); // Seller paid
        assert!(coin::balance<aptos_coin::AptosCoin>(purchaser_addr) == 9500, 0); // Purchaser charged
