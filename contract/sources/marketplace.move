module marketplace_addr::marketplace {
    ...
    const EMARKETPLACE_LISTING_DISABLED: u64 = 3;
  ...
  public entry fun list_with_fixed_price<CoinType>(
        ...
        // Verify the token is from our launchpad and listing is enabled
        let collection_addr = token::collection_object(object);
        let collection_obj = object::address_to_object<Collection>(collection_addr);
        assert!(
            launchpad::is_marketplace_enabled(collection_obj),
            error::permission_denied(EMARKETPLACE_LISTING_DISABLED)
        );
